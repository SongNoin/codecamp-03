...
function solution(new_id) {
  ...
  for (let i = 0; i < id.length; i++) {
    if (/\w/.test(id[i]) || id[i] === "-" || id[i] === ".") answer.push(id[i]);
  }
  ...
  // 4단계
  while (answer[0] === ".") answer = answer.slice(1, answer.length);
  ...
  // 7단계
  while (answer.length <= 2) answer = answer + answer[answer.length - 1];
